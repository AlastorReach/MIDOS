...
public final class CD {
    ...
    private static void SetPrompt(String actualPrompt, String prompt, String SP, String GP) {
        switch (actualPrompt) {
            case "$p":
                prompt = Arbol.getRutaActual() + SP;
                Singleton.getInstance().helper.SetPrompt(prompt);
                break;
            case "$g$p":
                prompt = GP + Arbol.getRutaActual();
                Singleton.getInstance().helper.SetPrompt(prompt);
                break;
            case "$p$g":
                prompt = Arbol.getRutaActual() + SP + GP;
                Singleton.getInstance().helper.SetPrompt(prompt);
                break;
            default:
                break;
    ...
    private static void goBackOneLevel(String actualPrompt, String prompt, String SP, String GP) {
        ...
        //si la carpeta actual es la raiz
        if (Singleton.getInstance().helper.getCarpetaActual().getNombre().equalsIgnoreCase("M:")) {
    ...
    // se ejecuta cuando el usuario digita CD/ o CD\
