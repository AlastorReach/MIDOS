...
public final class Prompt {
    ...
    public static void Prompt(List<String> parts, boolean isValidParam, String noValidCommand,
        ...
        switch (parameters.toUpperCase()) {
            case "$P":
                PromptP(prompt, Arbol.getRutaActual(), SP, actualPrompt);
                break;
            case "$G":
                PromptG(prompt, GP, actualPrompt);
                break;
            case "$P$G":
                PromptPG(prompt, Arbol.getRutaActual(), GP, SP, actualPrompt);
                break;
            case "$G$P":
                PromptGP(prompt, Arbol.getRutaActual(), GP, SP, actualPrompt);
                break;
            case "":
                PromptPG(prompt, Arbol.getRutaActual(), GP, SP, actualPrompt);
                break;
            default:
                Singleton.getInstance().error.printError("noCommand", parameters, 1);
                break;
        }
    }
