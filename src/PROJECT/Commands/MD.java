...
public final class MD {
    ...
    private static boolean DirectoryNameIsOk(String name, String type){
        if(name.matches("[a-zA-Z][a-zA-Z0-9_]*") != true){
            ...
            case "DIR": Singleton.getInstance().error.printError("errorNameDir", name ,0);return false;
            case "FIL": Singleton.getInstance().error.printError("errorNameFile", name ,0);return false;
            }
        }
        return true;
    }
    ...
    private static boolean DirectoryLengthIsOk(String name, String type){
        ...
        switch(type.toUpperCase()){
            case "DIR": Singleton.getInstance().error.printError("lengthDir", "" ,0);return false;
            case "FIL": Singleton.getInstance().error.printError("lengthFile", "" ,0);return false;
        }
        return false;
    }
}
